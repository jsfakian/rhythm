...
def process_v2_batch_item(job: "UploadJob", archive_path: str, extract_dir: str) -> None:
    ...
    # No "missing_patient_id" hard-fail: per the authoritative anonymization
    # tool (github.com/jsfakian/dicom_anonymization, GDPR-strict_explicit.json),
    # PatientID's directive is null ("remove") — a properly-anonymized
    # archive is expected to have NO PatientID at all, not a pseudonym.
    # GDPRAnonymizationValidator (gdpr_validator.py) now enforces exactly
    # that per-file. Its value/presence is never trusted for anything here
    # either way (see below) — a partner's tool once wrote the same literal
    # placeholder, "PID1", for every patient, which would have silently
    # merged distinct real patients into one Patient record under the old
    # scheme. dicom_patient_id is kept only for logging.
    study_uid = study_uids.pop()
    dicom_patient_id = patient_ids.pop() if patient_ids else ""

    # Assign (or reuse) the Repository Study ID *before* processing images —
    # this, not the partner-supplied DICOM PatientID, is now the platform's
    # own pseudo-identifier: generated by us, guaranteed unique by the
    # DB-locked sequence counter (or reused verbatim via
    # repository_study_id_override for a job created synchronously by
    # Manual Exam Entry), and already in the required pseudo-ID format by
    # construction. Note this means Patient identity is now per-*study*,
    # not per-real-patient — the platform no longer attempts to recognize
    ...
    PseudoIDUniquenessValidator.log_pseudo_id_tracking(repository_study_id, str(job.id))
    logger.info(f"DICOM-supplied PatientID for this archive (not used for identity): {dicom_patient_id}")
    ...
    for image_path in dicom_files:
        try:
            with transaction.atomic():
                # pseudo_id=None: don't enforce that the file's own PatientID
                # equals a specific value (we no longer trust/require that —
                # see repository_study_id above). GDPRAnonymizationValidator
                # still unconditionally requires *some* non-empty PatientID
                # tag to be present.
                is_valid, validation_errors = validate_gdpr_anonymization(image_path, pseudo_id=None)
                if not is_valid:
